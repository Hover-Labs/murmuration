...
export const deploy = async (params: DeployParams): Promise<void> => {
  ...
  for (let i = 0; i < params.vestingContracts.length; i++) {
    ...
    const vestingVaultStorage =  `(Pair (Pair (Pair ${vestingContract.amountPerBlock.toFixed()}      0) (Pair "${daoDeployResult.contractAddress}"               "${keystore.publicKeyHash}")) (Pair (Pair {Elt "" 0x74657a6f732d73746f726167653a64617461; Elt "data" } "${vestingContract.owner}") (Pair ${vestingContract.vestingStartBlock.toFixed()} "${tokenDeployResult.contractAddress}")))`                                 
